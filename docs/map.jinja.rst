...
We will see a quick setup to configure the ``nomad`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``nomad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``nomad`` formulas under ``/srv/salt/nomad/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/nomad/parameters/dns:domain/
    ln -s dns:domain /srv/salt/nomad/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/nomad/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/nomad/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``nomad`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``nomad`` formulas under ``/srv/salt/nomad/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/nomad/parameters/roles
...
We create a configuration for the role ``nomad/server`` in ``/srv/salt/nomad/parameters/roles/nomad/server.yaml``:
...
We create another configuration for the role ``nomad/client`` in ``/srv/salt/nomad/parameters/roles/nomad/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles nomad/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles nomad/client
...
.. code-block:: console
    ...
    mv /srv/salt/nomad/parameters/dns:domain/  '/srv/salt/nomad/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-nomad-as-json:
      test.nop:
        - name: {{ nomad | json }}
...
Here is an example based on `template-formula/nomad/config/file.sls`_:
...
This ``sls`` file expose a ``nomad`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ nomad['winner'] }}
...
.. _template-formula/nomad/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/nomad/config/file.sls
